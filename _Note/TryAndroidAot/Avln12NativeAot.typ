...
- 不修改 Ngan.Dict 現有項目的 Avalonia 版本。
- 不修改 `Ngan.Dict.Android.csproj` 中的 Avalonia 套件引用。
...
Ngan.Dict 使用 Avalonia 11.3.8 生成的 Android NativeAOT APK 可以完成編譯、打包、簽名和安裝，但在真機啓動時停留在啓動畫面。

`adb logcat` 已確認直接原因不是通知權限，也不是 Ngan.Dict 業務初始化，而是 Avalonia Android accessibility 初始化階段調用了 NativeAOT 不支持的 Reflection.Emit：
...
```text
Ngan.Dict.Frontend/proj/Ngan.Dict.Android/MainActivity.cs:61
```
...
因此，可以確定 Avalonia 12.1.0 加上本次使用的新版 AndroidX bindings，已經避開 Ngan.Dict/Avalonia 11.3.8 當前遇到的 accessibility JNI 註冊崩潰。
...
- 本次只測試了最小模板，尚未測試 Ngan.Dict 的完整依賴、業務代碼、SQLite、網絡、通知、音頻及其他 Android 功能。
...
它不在 `E:\_code\CsNgan.Dict` 中，因此不會修改 Ngan.Dict 項目文件、NuGet 中間產物或 Avalonia 版本。
...
因爲它使用獨立 package ID，所以安裝和測試不需要卸載或覆蓋 `Tsinswreng.Ngan.Dict`。
...
- `--artifacts-path`：將測試輸出放在 WSL ext4 的獨立目錄，避免和 Windows 或 Ngan.Dict 的 `bin/obj` 混用。
...
本次測試 package 與 Ngan.Dict 不同，所以沒有遇到 Ngan.Dict CoreCLR APK 與 NativeAOT APK 簽名不一致的覆蓋安裝錯誤。
...
#table(
  ...
  inset: 6pt,
  stroke: 0.5pt,
  [套件], [Ngan.Dict / Avalonia 11], [Avalonia 12.1 測試],
...
因此，升級 Avalonia 12 有實際希望解決 Ngan.Dict 當前的啓動阻塞。

=== 不能直接在現有項目盲目升級

本次沒有修改 Ngan.Dict 的 Avalonia 版本。是否升級需要單獨評估：
...
- Ngan.Dict 啓動後是否還會遇到下一個 NativeAOT 不兼容依賴。

最小模板成功只證明「框架最小路徑可行」，不能證明「Ngan.Dict 無需修改即可升級並運行」。

=== 不建議只在 Ngan.Dict 11 中強制覆蓋 AndroidX 版本
...
如果需要判斷最小升級範圍，應另建 Avalonia 11 的獨立測試項目，只提升 AndroidX bindings，再和 Avalonia 12 結果對照；不要直接拿 Ngan.Dict 主項目試錯。
...
=== 建立 Ngan.Dict 的 Avalonia 12 升級分支或副本
...
=== Ngan.Dict NativeAOT 功能測試

即使 Ngan.Dict 能進入主界面，仍至少需要驗證：
...
- `E:\_code\CsNgan.Dict\Ngan.Dict.Frontend\proj\Directory.Packages.props`
...
Avalonia 12.1.0 對 Android NativeAOT 的實際兼容性明顯好於 Ngan.Dict 當前使用的 Avalonia 11.3.8。
...
並且沒有重現 Ngan.Dict/Avalonia 11 的 Reflection.Emit accessibility 崩潰。

因此，若目標是讓 Ngan.Dict Android NativeAOT 真正啓動，Avalonia 12 是一條有實測依據、值得繼續驗證的路線。

但在完成手機解鎖後的可見 UI/交互驗證，以及 Ngan.Dict 全量依賴的 NativeAOT 測試之前，仍應將其視爲實驗方案，而不是已完成的生產遷移方案。
