...
也可能有`.Decl`省略不寫的情況、即 `Xxx.cs` vs `Xxx.Impl.cs`的情況
...
新寫的代碼也要遵從聲明與實現分離的規範、先寫聲明、等用戶確認聲明無誤後再實現。 當使用 聲明與實現分離的模式時、聲明部分的文件寫在 `Xxx.cs`(記得`.Decl`默認省略!)、有一點類似C語言的頭文件但不完全一樣;
...
- 你先寫完Decl的內容後讓用戶審查、審查通過後再寫Impl。 當你寫好Decl但沒寫Impl時、你去跑編譯肯定多半是跑不通的、不要管他、等你把Impl寫好後再一起跑編譯解決編譯錯誤
