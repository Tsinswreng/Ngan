...
`<項目根目錄>/.Tsinswreng/AiMemory/` 下爲全局AI記憶。 其中 `<項目根目錄>/.Tsinswreng/AiMemory/Main.typ`爲主文檔入口

## 何時讀取記憶

每次執行任務前先看一遍主文檔入口 `Main.typ`。 `Main.typ`中可能會引用其他文檔、你按需讀取。
...
`Main.typ`爲記憶入口。 由于記憶入口爲必讀項、所以一般我們不在`Main.typ`放置大量內容、而是將他作爲摘要和目錄使用。 具體內容放到其他文件夾下、然後在`Main.typ`中用`#link`函數引用。
